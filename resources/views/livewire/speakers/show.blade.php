<div>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-12">
        <div class="mb-6 flex justify-between items-center">
            ...
            <div>
                <flux:button
                    href="{{ route('events.speakers.edit', ['eventId' => $eventId, 'speakerId' => $speakerId]) }}"
                    icon="pencil">
                    Edit Speaker
                </flux:button>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1">
                <flux:card>
                    <div class="text-center">
                        ...
                        @else
                            <div
                                class="w-48 h-48 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center mx-auto mb-4">
                                <span
                                    class="text-6xl text-gray-500 dark:text-gray-400 font-semibold">{{ substr($speaker->name, 0, 1) }}</span>
                            </div>
                        @endif
                ...
                <!-- History (Collapsible) -->
                <div
                    class="text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg mt-6"
                    x-data="{ open: false }">

                    <button @click="open = !open"
                            class="w-full p-6 text-left flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                                            ...
                                                            @foreach($log->changes as $field => $change)
                                                                <p class="text-xs text-gray-600 dark:text-gray-300">
                                                                    ...
                                                                    @if(isset($change['old']) && isset($change['new']))
                                                                        <span
                                                                            class="text-red-600 dark:text-red-400">{{ $change['old'] ?: '(empty)' }}</span>
                                                                        →
                                                                        <span
                                                                            class="text-green-600 dark:text-green-400">{{ $change['new'] ?: '(empty)' }}</span>
                                                                    @else
                                                                        {{ json_encode($change) }}
                                                                    @endif
                                                                </p>
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    @forelse($speaker->sessions as $session)
                        <div class="border-b border-gray-200 dark:border-gray-700 py-3 last:border-0">
                            <div class="flex justify-between items-start">
                                ...
                                <flux:button size="sm"
                                             href="{{ route('events.sessions.show', ['eventId' => $eventId, 'sessionId' => $session->id]) }}"
                                             variant="ghost">
                                    View
                                </flux:button>
                            </div>
                ...
                <flux:card>
                    <flux:heading size="lg" class="mb-4">Content Files ({{ $speaker->contentFiles->count() }})
                    </flux:heading>
                        ...
                        <div class="border-b border-gray-200 dark:border-gray-700 py-3 last:border-0">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex gap-2">
                                    <flux:button size="sm"
                                                 href="{{ route('events.content.show', ['eventId' => $eventId, 'contentId' => $file->id]) }}"
                                                 variant="ghost">
                                        View
                                    </flux:button>
                                    <flux:button size="sm" href="{{ $file->download_url }}" variant="ghost"
                                                 target="_blank">
                                        Download
                                    </flux:button>
                                </div>
